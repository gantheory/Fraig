...
CirMgr::strash()
{
  HashMap< HashKey, WrapperGate > hash( getHashSize( _netList.size() ) * 10 );
  for ( size_t i = 0 , n = _netList.size() ; i < n ; ++i ) {
    if ( _netList[ i ]->gateType() == AIG_GATE ) {
      HashKey k( _netList[ i ]->invert( 0 ), _netList[ i ]->invert( 1 ), 
                 _netList[ i ]->_faninList[ 0 ]->getId(), 
                 _netList[ i ]->_faninList[ 1 ]->getId() );
...
bool
CirMgr::proofSATGrp( SatSolver& solver, vector< CirGate* >& fecGrp ) {
  vector< Var > newV;
  for ( int i = 0, in = fecGrp.size() - 1; i < in ; ++i ) 
    newV.push_back( solver.newVar() );
  for ( int i = 1, in = fecGrp.size() ; i < in ; ++i ) {
    if ( i == 1 ) {
      if ( fecGrp[ 0 ]->getSim() == fecGrp[ 1 ]->getSim() )
        solver.addAigCNF( newV[ 0 ], fecGrp[ 0 ]->getVar(), true, fecGrp[ 1 ]->getVar(), true );
      else 
        solver.addAigCNF( newV[ 0 ], fecGrp[ 0 ]->getVar(), true, fecGrp[ 1 ]->getVar(), false );
    }
    else {
      if ( fecGrp[ 0 ]->getSim() == fecGrp[ i ]->getSim() )
        solver.addAigCNF( newV[ i - 1 ], newV[ i - 2 ], false, fecGrp[ i ]->getVar(), true );
      else 
        solver.addAigCNF( newV[ i - 1 ], newV[ i - 2 ], false, fecGrp[ i ]->getVar(), false );
    }
  }
  solver.assumeRelease();  // Clear assumptions
  solver.assumeProperty( _totalList[ 0 ]->getVar(), false );
  solver.assumeProperty( newV[ newV.size() - 1 ], false);  // k = 1
  return !solver.assumpSolve();
}

void 
CirMgr::mergeGrp( vector< CirGate* >& fecGrp ) {
  cout << "Group" << endl;
  for ( int j = 1, jn = fecGrp.size() ; j < jn ; ++j ) {
    if ( fecGrp[ j ]->gateType() == AIG_GATE ) {
      if ( ( fecGrp[ 0 ]->_faninList[ 0 ] == fecGrp[ j ] ) ||
           ( fecGrp[ 0 ]->_faninList[ 1 ] == fecGrp[ j ] ) )
        SWAP( fecGrp[ 0 ], fecGrp[ j ] );
    }
    fraigMerge( fecGrp[ 0 ], fecGrp[ j ], fecGrp[ 0 ]->getSim() != fecGrp[ j ]->getSim() ); 
  }
  cout << "Group" << endl;
}
...
CirMgr::proofFecGrp( SatSolver& solver, vector< CirGate* >& fecGrp, 
  ...
  /*
  cout << "proof" << endl;
  if ( ( fecGrp.size() >= 3 ) && ( fecGrp.size() <= 6 ) ) { 
    cout << "Group" << endl;
    if ( proofSATGrp( solver, fecGrp ) ) { 
      mergeGrp( fecGrp );
      return true;
    }
